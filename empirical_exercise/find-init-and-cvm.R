...
Wscaled.kica <- vector("list", n_lhs)
Wscaled.spec <- vector("list", n_lhs)
w0          <- vector("list", n_lhs)
A.ica       <- vector("list", n_lhs)
A.kica       <- vector("list", n_lhs)
A.dc        <- vector("list", n_lhs)
A.spec        <- vector("list", n_lhs)
A.ID.pml    <- vector("list", n_lhs)
A.ID.dc     <- vector("list", n_lhs)
A.ID.ica    <- vector("list", n_lhs)
A.ID.kica    <- vector("list", n_lhs)
A.ID.pml    <- vector("list", n_lhs)
A.ID.spec    <- vector("list", n_lhs)
...
for (kk in 1:nrow(lhs)){
  ...
  kica_init[[kk]] <- KernelICA::kernel_ica(tu_chol, variant = "kgv", kernel = "gauss", eps = 1e-14,init = list(w0[[kk]]))
  
  Wscaled_kica[[kk]] <- rescaleVar(W_hat = kica_init[[kk]]$W, ut = u_chol)$Ws %*% solve(C)
  
  

  A.kica[[kk]] <- solve(Wscaled_kica[[kk]])
  ...
  cov_matrix <- cov(tu_chol)
  
  # Perform Cholesky decomposition
  chol_matrix <- chol(cov_matrix)
  ...
  W <- eig$vectors
  Wscal <- rescaleVar(W_hat = W, ut = u_chol)$Ws
  Wscaled_spec <- Wscal %*% solve(C)
  Wscaled.spec[[kk]] <- Wscaled_spec
  A.spec[[kk]] <- solve(Wscaled.spec[[kk]])
  
  
  
  # Continue with your loop or further processing
  ...
  A.ID.kica[[kk]]      <- maxfinder(A = A.kica[[kk]])$A.id
  #A.ID.chol[[kk]]      <- maxfinder(A = A.chol[[kk]])$A.id
  A.ID.spec[[kk]]      <- maxfinder(A = A.spec[[kk]])$A.id
...
c.kica <- vector("list", n_lhs)
c.dc  <- vector("list", n_lhs)
c.pml <- vector("list",n_lhs)
c.spec <- vector("list",n_lhs)
