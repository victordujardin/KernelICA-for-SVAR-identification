...
preliminary_table %>% pivot_wider(names_from = "estimator", values_from = c("GG","TT","XX")) %>% 
  dplyr::select(variable,contains("PML"),contains("fastICA"),contains("DCov"),contains("KernelICA")) %>% 
  ...
  add_header_above(c(" " = 1, rep(c("\\\\varepsilon_1" = 1 ,"\\\\varepsilon_2" = 1,"\\\\varepsilon_3" = 1),4)),
                     escape = F, line = F) %>% 
  add_header_above(c(" " = 1,  "$fastICA$" = 3, "$DCov$" = 3,"$KernelICA$" = 3, "$PML$" = 3),escape = F)

bind_cols(preliminary_table %>% filter(estimator %in% c("PML","DCov")) %>% dplyr::select(-estimator),
          preliminary_table %>% filter(estimator %in% c("fastICA","KernelICA")) %>% 
...
estimators <- c("fastICA","DCov","KernelICA")
